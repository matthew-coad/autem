...
renderPlot({
  ...
  df <- simulation_summary_rdf() %>% 
    ...
    select(experiment, study, progress)
...
renderPlot({
  ...
  output_df <-
    simulation_summary_df %>%
    mutate(
      validation_distance = (validation_score - rating) / rating_sd
    ) %>%
...
renderPlot({
  ...
  output_df %>%
    ggplot(aes(x = study)) +
    geom_errorbar(aes(ymin = rating-rating_sd, ymax=rating+rating_sd), width = .3, color = "black") +
    geom_errorbar(aes(ymin = rating-rating_sd * 2, ymax=rating+rating_sd* 2), width = .15, color = "black", linetype = 'dotted') +
...
renderPlot({
  ...
  step_detail_df %>%
    ggplot() +
    geom_point(aes(x = step, y = score, color = !!as.symbol(input$breakdown)), na.rm = TRUE) +
    geom_smooth(aes(x = step, y = score, linetype = "Training"), na.rm = TRUE, method = "loess", se = FALSE) +
    geom_smooth(aes(x = step, y = validation_score, linetype = "Validation"), na.rm = TRUE, method = "loess", se = FALSE) +
...
renderPlot({
  ...
  step_detail_df <- 
    step_detail_rdf() %>% 
    filtered_data(input$experiment) %>%
    filter(!is.na(score), !is.na(choice_predicted_score)) %>%
    group_by(member_id, league) %>%
    filter(step == min(step)) %>%
    mutate(
...
renderPlot({
  ...
  step_detail_df %>%
    ggplot(aes(x = step, y = standard_duration)) +
    geom_point(size = 0.5, aes(color = !!as.symbol(input$breakdown))) +
    facet_wrap(~ study)
})
