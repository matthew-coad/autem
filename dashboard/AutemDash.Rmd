...
renderPlot({
  ...
  step_detail_df %>%
    ggplot(aes(x = step)) +
    geom_point(aes(y = predicted_score, color = !!as.symbol(input$breakdown)), 
               na.rm = TRUE, alpha = .2, shape=16, size = 2) +
    geom_segment(
      aes(
        x = step, y = predicted_score + predicted_score_std, xend = step, yend = predicted_score -predicted_score_std
      ),
      na.rm = TRUE, alpha = .3, size = 1, color = "Grey") +
...
renderPlot({
  ...
  choices_score_df %>%
    ggplot() +
    geom_point(aes(x = component, y = score, color = choice, shape = "Score")) +
    geom_point(aes(x = component, y = predicted_score, color = choice, shape = "Predicted")) +
    coord_flip() +
    xlab("Component") +
    facet_wrap(~ study)
...
renderPlot({
  ...
  choice2_detail_df %>%
    ggplot() +
    geom_point(aes(x = component2, y = score, color = "Score"), na.rm = TRUE) +
    geom_point(aes(x = component2, y = predicted_score, color = "Predicted"), na.rm = TRUE) +
