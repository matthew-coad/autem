...
renderPlot({
  ...
  step_detail_df <- step_detail_rdf() %>% filtered_data(input$experiment)
...
renderPlot({
  ...
  durations_score_df %>%
    ggplot() +
    geom_point(aes(x = forcats::fct_reorder(component, choice, .fun = unique, .desc = TRUE), y = duration, color = choice)) + 
    coord_flip() +
    xlab("Component") +
    facet_wrap(~ study)
...
renderPlot({
  ...
  choice2_detail_df <-
    ...
    select(study, experiment, component1, component2, score)
  
  choice2_detail_df %>%
    ggplot() +
    geom_point(aes(x = component2, y = score)) +  
