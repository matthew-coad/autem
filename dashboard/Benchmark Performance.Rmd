...
summary_df <-
  ...
  select(experiment, dataset, version, ranking, rating, top_5p_accuracy, validation_accuracy, n_error) %>%
  ungroup()
...
baseline_accuracy <- function(baseline_name) {
  ...
  baseline_accuracy_df <- 
    ...
    select(experiment, dataset, version, accuracy)
...
for (baseline_name in baselines) {
  ...
  plot(ggplot(data = accuracy_df,aes(x = percentage, y = accuracy)) +
    ...
    geom_point(data = filter(accuracy_df, experiment != "baseline"), aes(shape = experiment, color = version), size = 3) +
    geom_rug(data = filter(accuracy_df, experiment != "baseline"), aes(color = version), sides="trbl") +
